...
So stations that are further than 20+ minutes away from multiple other stations (mostly would occur on the geographic edges of the system) could be at risk of draining or pooling. However, one way to avoid this is when citibike stations are placed next to other transit hubs to help riders commute their last leg. It helps riders avoid the extra cost of using the subway/vehicle for the last portion of their trip. In fact, the top 25 busiest stations were mostly located near transit hubs like Grand Central, Penn Station, Union Square, PATH stations, Columbus Circle:

![top25](images/top25.png)

This proximity of the top 25 to other transit hubs indicates that introducing new riders to the system who live outside the city or deep in the outer boroughs who use citibike for the last leg of their commuteboy, may be the cause of cause some of the drainage we seen in midtown.
...
The `app.py`, `index.py`, `layouts.py`, and `callbacks.py` in this repository create an interactive dashboard app for citibike to use to monitor the system. The Dashboard provides visualizations of station trends, live bike stats, station models generated by facebook prophet and a clustering map. [A demo of the dashboard is available here.](https://www.loom.com/share/ccc1286476194f0c8ffd07a21740813a)
